...
description: 对已生成或已提交的网文文本执行结构化体检，产出可执行的"诊断卡"。触发于用户要求"检查 / 点评 / 挑错 / 自检 / 是不是 AI 写的 / AI 味 / 哪里不对 / 这段问题在哪"等，或在其他子 skill 输出正文之前自动调用作为发布前检查。覆盖两部分：Part A 结构 10 项（矛盾 / 主角 / 爽点 / 大纲 / 钩子 …），Part B 反 AI 味 17 主条款 + 7 子条款 · 共 24 项（节拍器 / 句式复用 / 主语熵 / 单句段瀑布流 / 世界无自主生活含闲笔密度/配角议题/废选项戏剧化 / 情感标签 含独段与粗体 / 反应套餐 / signature 明牌化 / 设定说明书 / 9 种定义体模板 / PPT 直讲 / 三项并列讲解 / 算法思维链 / 爽点链条过完整 / 质量曲线过稳定 + 粗体主题句点题 / 角色灵魂缺位 / 剧情算法化含怪异预算/废选项/延迟兑付/反套路检查 / 转场机械含 5 类桥/禁用转场词/摩擦点必填）。长篇场景可直接读取 webnovel-memory 的 chapters/*、foreshadow.md、used-excitement.md、used-patterns.md、anti-trope-log.md 做跨章一致性体检（含灵魂渗透缺位名单、动物纯工具化名单、definition_style_hits、bold_theme_hits、signature_明牌超限名单、transition_types 分布、filler_density、side_char_autonomous_agenda 名单）。触发词：检查、点评、挑错、诊断、AI 味、是不是 AI、体检、audit。
...
| O | 角色灵魂缺位 | 关键角色首次登场章无灵魂渗透；任一有名出场 ≥ 2 次角色无灵魂渗透；动物 100% 服务主角；反派仅"想赢 / 恨"；配角可互换 |
...
| O | 每个关键 / 出场 ≥ 2 次有名角色各有 ≥ 1 处灵魂渗透；动物独立反应 ≥ 50%；反派有第三维度 | 主角灵魂渗透齐但配角 / 反派 / 动物缺 1 人 | 多数角色只剩功能；反派仅"想赢 / 恨"；动物纯工具化 | 关键角色（主角 / POV / 女男主 / 核心反派）首次登场章无灵魂渗透（退回 story-blueprint 补 soul_fields） |
...
STEP 3  Part B 17 主条款 + 7 子条款（24 项）逐条判定（引用 anti-ai-tells.md）
STEP 4  每个 FAIL 给"最小改法"（1–2 句话）+ 原文摘录；回滚级 FAIL 必须标出退回哪个 workflow
STEP 5  综合判定 + 优先级排序
STEP 6  输出"AI 味判定"：
        - 任一"回滚级 FAIL" → 极高 AI 味（必须整章退回指定 workflow 重做）
        - Part B FAIL ≥ 4 或 {D, M, N, N-细, O, G+1, E 粗体, P, Q} 任一 FAIL → 极高 AI 味
        - FAIL = 2–3 → 中度
        - FAIL = 1 且不含上述高危条款 → 偏低
        - FAIL = 0 且 WARN ≤ 3 → 低
```
...
- 最优先修复（按影响力排序）：
- 建议交接 skill：
  - 修结构（#A1–A4, #A6–A9, Part B 的 A/H/J/M/D-2/D-3/P-3/P-4/Q）→ `webnovel-plot-design`
  - 修文笔/情感/设定表达（#A5, #A10, Part B 的 B/B+/C/D-1/E/E+1/E+2/F/G/G+1/G+2/G-细/H+/I/K/N/N-细）→ `webnovel-excitement-and-craft`
...
- 若 Part B 出现 ≥ 4 条 FAIL，或 **D/M/N/P/Q** 任一回滚级 FAIL，诊断结论必须明确写"极高 AI 味，另一台 LLM 能抓出"。
- 不做作者心态类评价。
