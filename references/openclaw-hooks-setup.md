...
7. 若启用“隐性时序连续性”链路，需同步包含 `temporal_anchor_consistency_score` / `seasonal_sensory_conflict_hits` / `implicit_time_transition_bridge_hits` / `hard_timestamp_overuse_hits`；缺字段或冲突命中按 `write-protocol` 一致性检查 FAIL 处理。
8. 若启用“章首模板池（A-补充2）”链路，需同步包含 `opening_entry_mode` / `opening_mode_reason_code` / `opening_mode_fallback_used` / `opening_mode_streak_max_5ch` / `wakeup_opening_count_10ch` / `prev_hook_carryover_present` / `time_skip_bridge_present`；缺字段、模板三连、醒来开头过载或承接/跳时桥接不合规按 `write-protocol` 一致性检查 FAIL 处理。
