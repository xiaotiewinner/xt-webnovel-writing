...
8. 感情内容可写，且在新书/设计/续写前必须先问并记录：`romance_target_ratio` / `erotic_tension_target_ratio` / `explicitness_target_ratio`。在过审底线内优先满足用户的张力目标，不得默认压到接近零。
9. 允许“高张力但可过审”写法：以关系推进、反应失控、后果余波承载；禁止器官细节、体液细节、具体性行为步骤与教程式动作链。命中露骨或高风险关系时必须降级改写，但不得把整段感情戏删除为功能空壳。
10. 凡出现未成年人相关、强迫/胁迫、师生权力滥用、血缘乱伦等高风险关系表达，一律判回滚级失败，不得落盘。
...
感情线必须先问占比（感情/色情张力/露骨强度），在过审底线内尽量满足目标；禁止露骨步骤化描写与教程式动作链；命中高风险关系（未成年人/强迫/权力滥用/血缘乱伦）一律回滚拒收。
