...
## 定义体命中监控（反 G+1）
| 章 | 9 模板总命中数 | 命中模板清单 | 是否达标 |
|---|---|---|---|
| 85 | 1 | 不是 A 是 B ×1 | ✓ |
| 86 | 3 | 不是 A 是 B ×2；X 意味着 Y ×1 | ✗ WARN |
| 87 | 0 | — | ✓ |

- 阈值：单章 ≤ 2 次（PASS/WARN）；3–4 次 FAIL；≥ 5 次**回滚级 FAIL**（整章退回 plot-design 重写）
- 连续 2 章 WARN → 下一章禁用整个"是 / 不是 / 而是"系列
- 任一章回滚级 FAIL → 本章不得落盘
